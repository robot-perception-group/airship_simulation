...
class LongitudinalAutoPilot():
    ...
    def pitch_loop(self, pitch, pitch_rate, pitch_cmd):
        ...
        elevator_cmd = self.pitch_k_p * pitch_err - self.pitch_k_d * pitch_rate

        if (elevator_cmd > self.max_elevator):
            elevator_cmd = self.max_elevator
        else:
            if (elevator_cmd < -self.max_elevator):
                elevator_cmd = -self.max_elevator
    ...
    def altitude_loop(self, altitude, altitude_cmd, dt):
        ...
        altitude_err = altitude - altitude_cmd

        self.altitude_err_cur = self.altitude_err_cur + altitude_err * dt
        if (self.altitude_err_cur > self.integral_upper_bound):
            self.altitude_err_cur = self.integral_upper_bound
        else:
             if (self.altitude_err_cur < self.integral_lower_bound):
                self.altitude_err_cur = self.integral_lower_bound

        pitch_cmd = self.altitude_k_p * altitude_err + self.altitude_k_i * self.altitude_err_cur
        if (pitch_cmd > self.max_pitch_cmd):

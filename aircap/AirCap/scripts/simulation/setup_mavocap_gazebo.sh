...
screen -d -m -S GAZEBO bash -i -c "roslaunch rotors_gazebo world.launch world_name:=$WORLD  gui:=True --screen"
...
for i in $(seq 0 $(($ROBOS-1))); do
	...
	screen -d -m -S blimp$id bash -i -c "conda activate py2; roslaunch blimp_description blimp_without_env.launch roboID:=$id Z:=8 X:=${Xs[$i]}  Y:=${Ys[$i]} --screen"
